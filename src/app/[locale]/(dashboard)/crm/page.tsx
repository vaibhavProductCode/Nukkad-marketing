...
const TABS: { key: ContactSegment | "all"; label: string; icon: string }[] = [
  { key: "all", label: "Sab", icon: "👥" },
  { key: "champion", label: "Champions", icon: "⭐" },
  { key: "loyal", label: "Loyal", icon: "💚" },
  { key: "at_risk", label: "Wapas Laao", icon: "⚠️" },
  { key: "new", label: "Naye", icon: "🆕" },
    ...
    <div className="p-4 max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">{MOCK_CONTACTS.length} contacts · {MOCK_CONTACTS.filter((c) => c.optedInBroadcast).length} broadcast ke liye ready</p>
        </div>
        <button
      ...
      {/* WhatsApp import pitch card */}
      {showImport && (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 mb-4">
          <div className="text-2xl mb-2">📲</div>
          <p className="font-bold text-slate-800 text-sm mb-1">Apni WhatsApp chat yahan upload karein</p>
          <p className="text-xs text-slate-500 mb-3">Nukkad aapke customers ko samjhega — kaun loyal hai, kaun wapas aane wala hai.</p>
          <div className="bg-[#fff1eb] rounded-xl p-3 text-xs text-slate-600 mb-3">
            🔒 <strong>Aapki chats kabhi store nahi hoti</strong> — sirf signals nikal rahe hain, messages save nahi hote.
          </div>
          <button className="w-full bg-[#25D366] text-white font-bold py-3 rounded-xl text-sm hover:bg-green-600 transition-all">
            WhatsApp Export Upload Karein
          </button>
        ...
        {TABS.filter((t) => t.key !== "all").map((tab) => {
          ...
          return (
            <div key={tab.key} className="bg-white rounded-xl p-2.5 text-center border border-slate-100 shadow-sm">
              ...
              <p className="text-[10px] font-medium leading-tight" style={{ color: color?.text }}>{tab.label}</p>
            </div>
          );
        })}
      </div>

      {/* Tabs — WhatsApp chat filter style */}
      ...
      {/* Contact list — WhatsApp chat list style */}
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden mb-4">
        {filtered.length === 0 ? (
          <div className="p-12 text-center text-slate-400 text-sm">Is segment mein abhi koi contact nahi.</div>
        ) : (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-400 mt-0.5 truncate">
                  Last order: {contact.lastOrderAt} · {contact.purchaseCategories[0]}
                </p>
              </div>
              <button
        ...
        <div className="max-w-2xl mx-auto">
          <button className="pointer-events-auto w-full flex items-center justify-center gap-2 bg-[#16a34a] text-white font-black py-3.5 rounded-2xl shadow-lg text-sm hover:bg-[#15803d] transition-all">
            ...
            {selectedIds.length > 0 && (
              <span className="bg-white/20 text-xs px-2 py-0.5 rounded-full">{selectedIds.length} selected</span>
            )}
          </button>
        </div>
      </div>
